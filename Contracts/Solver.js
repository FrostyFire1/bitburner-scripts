...
async function sanitizeParentheses(ns, str)
{
    ns.tprint("sanitizeParentheses()");
    await ns.sleep(250);

    return removeInvalidParenthesis(str, ns);
}

// method checks if character is parenthesis(open
// or closed)
function isParenthesis(c, ns)
{
    return ((c == '(') || (c == ')'));
}
...
function removeInvalidParenthesis(str, ns)
{
    if (str.length==0)
        return;
